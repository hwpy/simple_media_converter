...
class AppButton:
    ...
    def open_file_dialog(self, window: QMainWindow):
        print("Clicked open_file_dialog")
        ...
        if dialog.exec():
            ...
            if filenames:
                if window.file_list.count() > 0:
                    items = [window.file_list.item(file).text() for file in range(window.file_list.count())]
                    window.file_list.addItems([str(Path(filename)) for filename in filenames if filename not in items])
                else:
                    window.file_list.addItems([str(Path(filename)) for filename in filenames])
    ...
    def convert_img(self, window: QMainWindow):
        ...
        for file in range(window.file_list.count()):
            ...
            heic_2_jpg.convert_file(cur_file)
            converted_files.append(cur_file)
        window.msg_converted.setText(f"Конвертация {window.file_list.count()} файлов завершена")
